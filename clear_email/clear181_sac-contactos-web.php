...
echo "Error: No se ha podido conectar a la base de datos\n";
	...
	$query_sql = 'SELECT DISTINCT(web.documento),web.apyn,web.email from sac.contacto_web web
					INNER JOIN cidig.persona cid on cid.cuil_documento::bigint = web.documento
					WHERE web.email is not null ';
	...
	while ($persona = pg_fetch_array($personas)) {
		...
		if ($persona['email'] != null){
			...
			$query_sim = "SELECT DISTINCT(web.documento),simP.*  FROM cidig.persona AS simP
							INNER JOIN sac.contacto_web web on simP.cuil_documento::bigint = web.documento 
