...
<?php
   ...
   if(!$db) {
      ...
      $query1 = "SELECT cp.id_persona, ts.nombre, ts.tel_contacto 
                  FROM turnos.solicitudes ts
                  INNER JOIN cidig.persona cp ON cp.cuil_documento = ts.dni::text
                  WHERE (cp.id_persona NOT IN 
                     (SELECT ctp.id_persona FROM cidig.telefono_persona ctp) AND (ts.tel_contacto IS NOT NULL)) ";
